...
def map_scores(dqfd_scores=None, ddqn_scores=None, dqn_scores=None, xlabel=None, ylabel=None):
    if dqfd_scores is not None:
        plt.plot(dqfd_scores, 'r')
    if ddqn_scores is not None:
        plt.plot(ddqn_scores, 'b')
    if ddqn_scores is not None:
        plt.plot(dqn_scores, 'g')
...
if __name__ == '__main__':
    ...
    map_scores(dqfd_scores=dqfd_mean_scores, ddqn_scores=ddqn_mean_scores, dqn_scores=dqn_mean_scores,

        xlabel='Red: dqfd         Blue: ddqn        Green: dqn', ylabel='Scores')
    env.close()
